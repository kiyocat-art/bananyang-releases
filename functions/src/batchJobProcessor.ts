...
interface BatchJobItem {
    ...
    /** Storage 경로 기반 이미지 (신규). originalImageB64보다 우선 적용. */
    originalImagePath?: string;
    /** @deprecated Firestore 직접 저장 방식 (구형 잡 하위 호환용). */
    originalImageB64?: string;
    originalImageMime?: string;
}

interface BatchJobDocument {
    uid: string;
    status: string;
    items: BatchJobItem[];
    totalItems: number;
    ...
    results: Array<{ storageUrl: string; storagePath?: string; itemIndex: number }>;
    /** 사용자 개인 Gemini API 키 — 잡 제출 시 클라이언트가 전달 */
    userApiKey?: string;
    /** 막힌 pending 잡 재시도 신호 — detectStuckJobs가 갱신하면 onUpdate 트리거를 통해 재처리됨. */
    _retryAt?: admin.firestore.Timestamp;
}
...
async function generateImage(item: BatchJobItem, apiKey: string): Promise<Buffer> {
...
async function runJob(
    ...
    const results: Array<{ storageUrl: string; storagePath: string; itemIndex: number }> = [];
...
const RUN_WITH_CONFIG: functions.RuntimeOptions = {
    timeoutSeconds: 540,
    memory: '1GB',
};

function resolveApiKey(label: string, job: BatchJobDocument): string | null {
...
/** 신규 잡 생성 시 처리 */
export const processBatchJob = functions
    .runWith(RUN_WITH_CONFIG)
    .firestore.document('batch_jobs/{jobId}')
    .onCreate(async (snap, context) => {
        const jobData = snap.data() as BatchJobDocument;
 ...
 */
export const retryBatchJob = functions
    .runWith(RUN_WITH_CONFIG)
    .firestore.document('batch_jobs/{jobId}')
    .onUpdate(async (change, context) => {
        const before = change.before.data() as BatchJobDocument;
        const after = change.after.data() as BatchJobDocument;
        ...
        if (!statusChangedToPending && !retryRequested) return;
        if (after.status !== 'pending') return; // 안전 가드
