...
exports.cleanupExpiredJobs = exports.detectStuckJobs = void 0;
 ...
 *   - 48h 초과: status → 'failed'
 */
exports.detectStuckJobs = functions
    ...
    functions.logger.info(`[detectStuckJobs] Found ${processingSnap.size} stuck processing + ${pendingSnap.size} stuck pending job(s).`);
 ...
 */
exports.cleanupExpiredJobs = functions
...
async function deleteOldCompletedJobs(retentionThreshold) {
...
async function expireStaleActiveJobs(googleExpireThreshold) {
    const snapshot = await db()
        .collection('batch_jobs')
...
//# sourceMappingURL=batchMaintenance.js.map